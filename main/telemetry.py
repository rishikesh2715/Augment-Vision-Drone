...
def processSerialData(drone):
    ...
    #                   'Pitch', 'Roll', 'Yaw', 'Flight Mode', 'Destination', 'Origin', 'Device Name']
    ...
    #                 data.update({'Type': 'GPS', 'Lat': lat, 'Lon': lon, 'Speed': speed, 'Heading': heading, 'Altitude': altitude, 'Sats': sats})

    #             elif frame_type == 0x07:  # VARIO
    #                 vertical_speed = struct.unpack('>h', payload)[0]
    #                 print(f'VARIO: Vertical speed={vertical_speed}')
    #                 writer.writerow(["VARIO", f'Vertical speed={vertical_speed}'])
    ...
    #                 current /= 10          # amps
    #                 print(f'Battery: Voltage={voltage}, Current={current}, Used={used}, Remaining={remaining}')
    #                 data.update({'Type': 'Battery', 'Voltage': voltage, 'Current': current, 'Used': used, 'Remaining': remaining})
    ...
    #                 downlink_rssi /= -1
    #                 print(f'Link Statistics: Uplink RSSI Ant.1={uplink_rssi_ant1}dBm, Uplink RSSI Ant.2={uplink_rssi_ant2}dBm, Uplink Quality={uplink_quality}%, Uplink SNR={uplink_snr}db, Active Antenna={active_antenna}, RF Mode={rf_mode}, Uplink TX Power={uplink_tx_power}, Downlink RSSI={downlink_rssi}dBm, Downlink Quality={downlink_quality}%, Downlink SNR={downlink_snr}db')
    #                 data.update({'Type': 'Link Statistics', 'Uplink RSSI Ant.1': uplink_rssi_ant1, 'Uplink RSSI Ant.2': uplink_rssi_ant2, 'Uplink Quality': uplink_quality, 'Uplink SNR': uplink_snr, 'Active Antenna': active_antenna, 'RF Mode': rf_mode, 'Uplink TX Power': uplink_tx_power, 'Downlink RSSI': downlink_rssi, 'Downlink Quality': downlink_quality, 'Downlink SNR': downlink_snr})    
    ...
    #                 data.update({'Type': 'Attitude', 'Pitch': pitch, 'Roll': roll, 'Yaw': yaw}) 

    #             elif frame_type == 0x21:  # Flight Mode
    #                 flight_mode = payload.decode('ascii').rstrip('\x00')
    #                 print(f'Flight Mode: {flight_mode}')
    #                 data.update({'Type': 'Flight Mode', 'Flight Mode': flight_mode})

    #             elif frame_type == 0x29:  # Device Info
    #                 destination, origin = struct.unpack('<BB', payload[:2])
    #                 device_name = payload[2:].decode('ascii').rstrip('\x00')
    #                 print(f'Device Info: Destination={destination}, Origin={origin}, Device Name={device_name}')
    #                 data.update({'Type': 'Device Info', 'Destination': destination, 'Origin': origin, 'Device Name': device_name})

// telemetry source object for the Drone
...
function Drone() {
	...
	let rawDict = fs.readFileSync('../openmct/example/Drone/Dronedictionary.json')
			...
			try{ // store in history
				...
				if(this.continousLogging){					
					...
					this.asyncStringyify(message).then(function(write) {//write is the value of the resolved promise in asyncStringify
						fs.appendFile(__dirname + '/saved_logs/Drone_'+this.FileTimestamp+'_rawMessage.json', write, (err) => {
							if (err) {
								console.log(err);
							} 
						}) 
	...
	server.on('error', (err) => {
		console.log(`Drone UDP server error:\n${err.stack}`);
		try{
...
Drone.prototype.generateRealtimeTelemetry = function () {
...
Drone.prototype.generateIntervalTelemetry = function () {
...
Drone.prototype.notify = function (point) {
...
Drone.prototype.listen = function (listener) {
...
Drone.prototype.SetFileTimestamp = function () {
...
Drone.prototype.asyncStringyify = function (obj) {
...
Drone.prototype.command = function (command) {
	...
	if(command === ':saveHistory'){
		...
		this.asyncStringyify(this.history).then(function(write) {//write is the value of the resolved promise in asyncStringify
			fs.writeFile(__dirname + '/saved_logs/Drone_'+this.FileTimestamp+'_History.json', write, (err) => {
				if (err) {
					console.log(err);
				} else {
	...
	// 			fs.writeFile(__dirname + '/saved_logs/Drone_'+this.FileTimestam+'.json', write, (err) => {
	// 				if (err) {
	// 					throw err;
	// 				}
